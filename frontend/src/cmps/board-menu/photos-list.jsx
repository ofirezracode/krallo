...
export default function PhotosList({ board, setTitle, onUpdateBoardBg }) {
        ...
        <>
            {!imgs ? <div><Loader /></div> :
                <ul className='photos-list clean-list'>
                    {imgs.map((img, idx) => (
                        <li key={idx} className='flex justify-center column' onClick={(ev) => onSetBoardBg(img.urls.full, ev)}>
                            <div className='bg-img' style={{ background: `url(${(img.urls.small)}) center center / cover` }}>
                                <a href={img.user.portfolio_url} target='_blank'>{img.user.username}</a>
                            </div>
                        </li>))}
                </ul >}
        </>
