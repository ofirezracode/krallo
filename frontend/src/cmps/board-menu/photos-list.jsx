...
export function PhotosList({ onUpdateBoardBg, resultsAmount, returnSize, setSelectedImg, selectedImg }) {
  ...
  useEffect(() => {
    ...
    const fetchImgs = async () => {
      const response = await fetch(`https://api.unsplash.com/photos?&page=${utilService.getRandomIntInclusive(1, 10)}&per_page=${amount}&query=${text}&client_id=${process.env.REACT_APP_UNSPLASH_API_KEY}`)
